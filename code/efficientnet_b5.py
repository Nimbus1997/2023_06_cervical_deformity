# Efficient Net - pretrained but update all parameters
# edit 
...
# 실행방법
# 1) change 내용 변경
# 2) python 파일명.py (nohup python efficientnet_b5.py>../result/efficientnet_b5/efficientnet_b5_allclass_0627.txt&)
...
result_folder = "/root/jieunoh/cervical_deformity/result/efficientnet_b5"
result_name = "efficientnet_b5_allclass_0629"
...
model.classifier[1] = nn.Linear(model.classifier[1].in_features, num_classes) # 마지막 레이어를 num_classes에 맞게 수정
...
for fold, (train_idx, val_idx) in enumerate(kfold.split(dataset)):
    ...
    trainloader = torch.utils.data.DataLoader(dataset, batch_size=batchsize, sampler=train_subsampler) # 해당하는 index 추출
    valloader = torch.utils.data.DataLoader(dataset, batch_size=batchsize, sampler=val_subsampler)
